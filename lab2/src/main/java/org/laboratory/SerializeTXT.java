...
public class SerializeTXT
{
    public void serializer(Sportsman obj, File file) throws IOException
    {
        try(FileWriter fw = new FileWriter(file)){
            String str = "Firstname = " + obj.getFirstname() + ", Lastname = " + obj.getLastname() + ", Height = " + obj.getHeight()
    ...
    public Sportsman deserializer(File file) throws IOException {
        try (BufferedReader fr = new BufferedReader(new FileReader(file))) {
            ...
            a.SetSport(fields[2].split(" ")[2]);
            a.SetQualification(fields[3].split(" ")[2]);
            a.SetHeight(Integer.parseInt(fields[4].split(" ")[2]));
            a.SetWeight(Integer.parseInt(fields[5].split(" ")[2]));
            a.SetAge(Integer.parseInt(fields[6].split(" ")[2]));
            return a.build();
        }
        catch (IOException e) {
